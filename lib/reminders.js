...
function dailyMessage(habits, local, settings) {
  ...
  const note = saved.length ? `Dün ${saved.map((h) => h.name).join(', ')} kaçtı; seri koruyucu serini korudu. ` : '';
...
function weeklyMessage(habits, goals, local) {
  ...
  const best = habits.map((h) => [h, currentStreak(h, local.date)]).sort((a, b) => b[1] - a[1])[0];
